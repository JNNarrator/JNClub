...
<template>
  ...
  >
    ...
    <div class="info">
      ...
      <p v-if="player.currentTrack && player.loading" class="status-line loading-text">
        正在获取/缓冲音频…
      </p>
      <p v-else-if="player.currentTrack && player.getTrackState(player.currentTrack.trackId) === 'error'" class="status-line error-text" role="button" title="点击重试" @click.stop="player.retryCurrent()">
        播放失败，点击重试
      </p>
    </div>
...
.status-line.loading-text { color: var(--jn-accent, #f2b134); }
.status-line.error-text { color: var(--jn-danger, #f5485c); }
