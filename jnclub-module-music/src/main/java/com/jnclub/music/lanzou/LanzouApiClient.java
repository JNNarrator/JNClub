package com.jnclub.music.lanzou;

import com.google.gson.*;
import com.jnclub.music.lanzou.config.LanzouClientProperties;
import com.jnclub.music.lanzou.dto.*;
import okhttp3.*;
...
public class LanzouApiClient {
    ...
    private static final Logger log = LoggerFactory.getLogger(LanzouApiClient.class);
    ...
    private static final Pattern DIRECT_URL_EXPIRY_PATTERN = Pattern.compile("[?&]e=([0-9a-fA-F]{6,})");
    ...
    /** 连续自动登录失败次数（达到阈值后提示人工干预，避免频繁重试被风控）。 */
    private final java.util.concurrent.atomic.AtomicInteger loginFailCount = new java.util.concurrent.atomic.AtomicInteger(0);
    private static final int LOGIN_FAIL_THRESHOLD = 3;
    ...
    public boolean bootstrap() {
        synchronized (bootstrapLock) {
            ...
            try {
                ...
                sessionCookies.clear();
                uid = ""; vei = "";
                int fails = loginFailCount.incrementAndGet();
                String hint = fails >= LOGIN_FAIL_THRESHOLD
                        ? "（连续 " + fails + " 次自动登录失败，请手动更新 Cookie 后重试）"
                        : "（失败 " + fails + "/" + LOGIN_FAIL_THRESHOLD + " 次）";
                log.warn("[lanzou] 自动登录失败{}: {}", hint, e.getMessage());
    ...
    private LanzouDirectLink directLinkInternal(String fileId, String fileName) {
        ...
        return new LanzouDirectLink(downUrl, resolveRealExpiry(downUrl));
    }
    ...
    private LanzouDirectLink getFileDownloadLinkInternal(String fileId) {
        ...
        // Step 3: 解析直链
        String directUrl = resolveDownloadUrl(downloadUrl, refererBase);
        return new LanzouDirectLink(directUrl, resolveRealExpiry(directUrl));
    }
