 ...
 */
import { Bookmark, StickyNote, Cloud, KeyRound, Music, Trash2, Puzzle, LayoutDashboard, ListTodo, CalendarDays, GlobeLock } from 'lucide-vue-next'

/** 可配置导航项 key */
export type NavKey = 'overview' | 'bookmarks' | 'notes' | 'files' | 'vault' | 'todos' | 'calendar' | 'music' | 'recycle' | 'webdav' | 'extension'
...
export const DEFAULT_ORDER: NavKey[] = ['overview', 'bookmarks', 'notes', 'files', 'vault', 'todos', 'calendar', 'music', 'recycle', 'webdav', 'extension']

/** 移动端底部 TabBar 参与项（音乐/插件/WebDAV 页移动端不显示；日历纳入，横向滚动容纳更多入口） */
export const MOBILE_KEYS: NavKey[] = ['overview', 'bookmarks', 'notes', 'files', 'vault', 'todos', 'calendar', 'recycle']
