...
function onScroll(e: Event) {
  ...
  const scroller = document.querySelector('.track-scroll') as HTMLElement | null
  const inner = scroller && scroller.scrollHeight > scroller.clientHeight
  let nearBottom = false
  if (inner && scroller) {
    nearBottom = scroller.scrollTop + scroller.clientHeight >= scroller.scrollHeight - 200
  } else {
    const doc = document.documentElement
    nearBottom = window.innerHeight + (window.scrollY || doc.scrollTop || 0) >= doc.scrollHeight - 200
  }
...
function onWindowScroll() {
  const scroller = document.querySelector('.track-scroll')
  if (scroller && scroller.scrollHeight > scroller.clientHeight) return // 内部滚动由 onScroll 处理
  const doc = document.documentElement
  if (window.innerHeight + (window.scrollY || doc.scrollTop || 0) >= doc.scrollHeight - 200) {
    loadMore()
  }
}
      ...
      <div v-else class="track-rows">
        ...
        >
          <div class="row-play">
            ...
            <span v-else-if="player.getTrackState(track.trackId) === 'loading'" class="row-state loading">
              <el-icon class="spin" :size="16"><Loading /></el-icon>
            </span>
            <span v-else-if="player.getTrackState(track.trackId) === 'error'" class="row-state error" title="直链获取失败，点击重试">
              <el-icon :size="14"><WarningFilled /></el-icon>
            </span>
            <span v-else-if="player.getTrackState(track.trackId) === 'ready'" class="row-state ready" title="可播放">
...
.row-state.loading { color: var(--jn-accent, #f2b134); }
.row-state.ready { color: var(--jn-ok, #34c38f); }
.row-state.error { color: var(--jn-danger, #f5485c); }
